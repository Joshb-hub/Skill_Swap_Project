...
class Profile(Base):
    ...
    preferred_learning_mode: Mapped[LearningMode] = mapped_column(SAEnum(LearningMode), default=LearningMode.ONLINE)
    email_privacy: Mapped[PrivacyLevel] = mapped_column(SAEnum(PrivacyLevel), default=PrivacyLevel.VISIBLE_AFTER_ACCEPTANCE)
    phone_privacy: Mapped[PrivacyLevel] = mapped_column(SAEnum(PrivacyLevel), default=PrivacyLevel.HIDDEN)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now, onupdate=utc_now)
...
class UserSkill(Base):
    ...
    experience_level: Mapped[Optional[ExperienceLevel]] = mapped_column(SAEnum(ExperienceLevel), nullable=True)
    current_level: Mapped[Optional[ExperienceLevel]] = mapped_column(SAEnum(ExperienceLevel), nullable=True)
    target_level: Mapped[Optional[ExperienceLevel]] = mapped_column(SAEnum(ExperienceLevel), nullable=True)
    description: Mapped[Optional[str]] = mapped_column(Text, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), default=utc_now)
...
class SkillSwap(Base):
    ...
    skill_a_id: Mapped[str] = mapped_column(String(36), ForeignKey("skills.id", ondelete="CASCADE"))
    skill_b_id: Mapped[str] = mapped_column(String(36), ForeignKey("skills.id", ondelete="CASCADE"))
...
class LearningGoal(Base):
    ...
    id: Mapped[str] = mapped_column(String(36), primary_key=True, default=lambda: str(uuid.uuid4()))
    user_id: Mapped[str] = mapped_column(String(36), ForeignKey("users.id", ondelete="CASCADE"), index=True)
    skill_swap_id: Mapped[Optional[str]] = mapped_column(String(36), ForeignKey("skill_swaps.id", ondelete="CASCADE"), nullable=True, index=True)
    skill_id: Mapped[str] = mapped_column(String(36), ForeignKey("skills.id", ondelete="CASCADE"), index=True)
